...
export class DeflectionSolver {
  private beam: Beam;
  private smSolver: ShearMomentSolver;
  private numIntervals: number = 400;
  private xGrid: number[] = [];
  private thetaGrid: number[] = [];
  private vGrid: number[] = [];
  private c1: number = 0;
  private c2: number = 0;
  private EI: number = 1;

  constructor(beam: Beam, numIntervals: number = 400) {
    this.beam = beam;
    this.numIntervals = numIntervals;
    ReactionSolver.solve(this.beam);
    this.smSolver = new ShearMomentSolver(this.beam);
    this.computeDeflection();
  }

  private computeDeflection(): void {
    ...
    const interpolateI1 = (x: number): number => {
    ...
    const interpolateI2 = (x: number): number => {
      const clamped = Math.max(0, Math.min(L, x));
      const idx = Math.min(N - 1, Math.floor(clamped / h));
      const t = (clamped - this.xGrid[idx]) / h;
    ...
    const fixedSupport = supports.find((s) => s.SupportType === supportType.FIXED);
   ...
   */
  sampleDeflectionCurve(points: number = 100): { x: number; v: number }[] {
   ...
   */
  sampleSlopeCurve(points: number = 100): { x: number; theta: number }[] {
    const samples: { x: number; theta: number }[] = [];
    const step = this.beam.Length / points;
