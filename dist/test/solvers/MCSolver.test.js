...
describe("simple beam with pinned and roller supports -> middle load", () => {
    ...
    beforeAll(() => {
        ...
        const endSupport = new RollerSupport_1.RollerSupport(beam.Length); // Add a roller support at the end of the beam
...
describe("simple beam with pinned and roller supports -> deviated load", () => {
    let beam;
    beforeAll(() => {
        beam = new beam_1.Beam(17.5); // Create a beam of length 17.5
        const startSupport = new PinnedSupport_1.PinnedSupport(0); // Add a pinned support at position 0
        const endSupport = new RollerSupport_1.RollerSupport(beam.Length);
        beam.setSupports([startSupport, endSupport]);
        beam.addForce(new src_1.SimpleForce(6.5, "down", 9.9));
        const solver = new MCSolver_1.MCSolver(beam);
        solver.solveReactions();
    });
    it("Start Reaction", () => {
