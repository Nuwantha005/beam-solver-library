...
const BeamAnalyzer_1 = __importDefault(require("../../src/solvers/BeamAnalyzer"));
describe("BeamAnalyzer — Simply Supported Beam with Point Load and UDL", () => {
    ...
    it("should produce valid shear and moment diagrams with polynomial degrees", () => {
        const result = BeamAnalyzer_1.default.analyze(beam);
...
describe("BeamAnalyzer — Cantilever Beam with Tip Load", () => {
    it("should produce correct reactions and linear diagrams", () => {
        ...
        beam.addLoad(new PointLoad_1.default(20, 6));
...
describe("BeamAnalyzer — Discontinuity Detection", () => {
    it("should tag discontinuity at concentrated point load", () => {
        ...
        beam.setSupports([new PinnedSupport_1.PinnedSupport(0), new RollerSupport_1.RollerSupport(10)]);
        beam.addLoad(new PointLoad_1.default(30, 4));
        const result = beam.analyze();
        // Point load at x=4 causes discontinuity in shear
        const discPoint = result.shearDiagram.points.find((p) => p.isDiscontinuity && Math.abs(p.x - 4) < 1e-4);
        expect(discPoint).toBeDefined();
        expect(discPoint === null || discPoint === void 0 ? void 0 : discPoint.leftLimit).toBeCloseTo(18); // R1 = 30 * 6 / 10 = 18
        expect(discPoint === null || discPoint === void 0 ? void 0 : discPoint.rightLimit).toBeCloseTo(-12); // 18 - 30 = -12
    });
});
