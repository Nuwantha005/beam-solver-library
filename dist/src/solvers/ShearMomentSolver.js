...
exports.ShearMomentSolver = void 0;
...
class ShearMomentSolver {
    ...
    getShearAt(x, side = "exact") {
        ...
        this.beam.getLoads().forEach((load) => {
            if (load.loadType === ILoad_1.LoadType.POINT) {
            ...
            else if (load instanceof UniformlyDistributedLoad_1.default) {
            ...
            else if (load instanceof TaperzoidLoad_1.default) {
                if (clampedX > load.startLocation) {
                    const effectiveEnd = Math.min(clampedX, load.endLocation);
                    const activeLength = effectiveEnd - load.startLocation;
                    const totalSpan = load.endLocation - load.startLocation;
                    if (activeLength > 0 && totalSpan > 0) {
                        const wEnd = load.startLoad + (activeLength / totalSpan) * (load.endLoad - load.startLoad);
     ...
     */
    getZeroCrossings() {
        const events = BeamEventEngine_1.default.extractEvents(this.beam);
        const crossings = [];
        // 1. Check endpoints
    ...
    getMaxMoment() {
        const criticalPoints = [
            ...BeamEventEngine_1.default.extractEvents(this.beam),
...
exports.ShearMomentSolver = ShearMomentSolver;
exports.default = ShearMomentSolver;
