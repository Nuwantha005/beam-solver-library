...
import SimpleForce from "../../src/objects/Forces/SimpleForce";
import ReactionSolver from "../../src/solvers/ReactionSolver";
import {
  UnstableBeamError,
...
describe("ReactionSolver — Simply Supported Beams", () => {
  it("should solve symmetric midspan point load (L=10, P=20 at x=5)", () => {
    ...
    const pin = new PinnedSupport(0);
    const roller = new RollerSupport(10);
    ...
    expect((roller.Reaction as SimpleForce).getDirection()).toBe(90);
  });
  ...
  it("should solve asymmetric point load (L=10, P=30 at x=2)", () => {
    ...
    const roller = new RollerSupport(10);
    beam.setSupports([pin, roller]);
    beam.addLoad(new PointLoad(30, 2));
  ...
  it("should solve full UDL (L=8, w=4 kN/m)", () => {
    ...
    const pin = new PinnedSupport(0);
    const roller = new RollerSupport(8);
    beam.setSupports([pin, roller]);
  ...
  it("should solve triangular/trapezoidal load (L=6, w=0 to 6 kN/m)", () => {
    ...
    const pin = new PinnedSupport(0);
    const roller = new RollerSupport(6);
    beam.setSupports([pin, roller]);
  ...
  it("should solve angled 2D point load with horizontal reaction", () => {
    ...
    beam.addLoad(new Force2D(mag, angle, 5));
...
describe("ReactionSolver — Cantilever Beams", () => {
  it("should solve cantilever fixed at x=0 with tip point load (L=5, P=12 at x=5)", () => {
    ...
    const fixed = new FixedSupport(0);
  ...
  it("should solve cantilever fixed at right end x=L (L=6, P=10 at x=0)", () => {
    const beam = new Beam(6);
    const fixed = new FixedSupport(6);
    beam.setSupports([fixed]);
...
describe("ReactionSolver — Stability & Determinacy Validation", () => {
  it("should throw UnstableBeamError when fewer than 3 DOFs provided", () => {
    const beam = new Beam(10);
    beam.setSupports([new RollerSupport(0)]); // Only 1 DOF
    beam.addLoad(new PointLoad(10, 5));

    expect(() => ReactionSolver.solve(beam)).toThrow(UnstableBeamError);
  });
  ...
  it("should throw IndeterminateBeamError when more than 3 DOFs provided", () => {
    const beam = new Beam(10);
    beam.setSupports([
      new PinnedSupport(0), // 2 DOFs
      new PinnedSupport(10), // 2 DOFs => Total 4 DOFs
    ]);
    beam.addLoad(new PointLoad(10, 5));

    expect(() => ReactionSolver.solve(beam)).toThrow(IndeterminateBeamError);
  });
});
