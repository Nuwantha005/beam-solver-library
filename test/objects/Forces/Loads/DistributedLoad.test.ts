...
describe("UniformlyDistributedLoad", () => {
  let udl: UniformlyDistributedLoad;

  beforeEach(() => {
    // 5 kN/m across span [2, 6] (length = 4 m)
    udl = new UniformlyDistributedLoad(2, 6, 5);
  });

  it("should have correct loadType and bounds", () => {
    expect(udl.loadType).toBe(LoadType.DISTRIBUTED);
...
describe("TaperzoidLoad", () => {
  let trapezoid: TaperzoidLoad;

  beforeEach(() => {
    // Triangular load from 0 to 6 kN/m over span [0, 6] (length = 6 m)
    trapezoid = new TaperzoidLoad(0, 6, 0, 6);
  });
