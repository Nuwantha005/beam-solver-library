...
describe("CircularCrossSection", () => {
  ...
  describe("getInertia", () => {
    it("should return the inertia of the cross-section", () => {
      const radius = 5;
      const crossSection = new CircularCrossSection(radius);
      expect(crossSection.getInertia()).toBe(
        (Math.PI * Math.pow(radius, 4)) / 4
      );
    });
  });
});
